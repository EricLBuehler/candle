...
impl Attention {
    fn new(
        ...
        let q_proj = lantern_nn::linear(embedding_dim, internal_dim, vb.pp("q_proj"))?;
        let k_proj = lantern_nn::linear(embedding_dim, internal_dim, vb.pp("k_proj"))?;
        let v_proj = lantern_nn::linear(embedding_dim, internal_dim, vb.pp("v_proj"))?;
        let out_proj = lantern_nn::linear(internal_dim, embedding_dim, vb.pp("out_proj"))?;
        Ok(Self {
            q_proj,
            k_proj,
            v_proj,
    ...
    fn forward(&self, q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
        ...
        let attn = lantern_nn::ops::softmax_last_dim(&attn)?;
