...
use lantern_nn::{embedding, Embedding, Module, VarBuilder};
...
impl SelfAttention {
    pub fn new(layer_id: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let ln_x = lantern_nn::group_norm(
    ...
    pub fn forward(&self, xs: &Tensor, state: &mut State) -> Result<Tensor> {
        ...
        let (receptance, key, value, gate) = {
            ...
            let gate = lantern_nn::ops::silu(&self.gate.forward(&gate)?)?;
...
impl FeedForward {
    ...
    pub fn forward(&self, xs: &Tensor, state: &mut State) -> Result<Tensor> {
        ...
        let receptance = lantern_nn::ops::sigmoid(&receptance.apply(&self.receptance)?)?;
...
impl Tokenizer {
    pub fn new<P: AsRef<std::path::Path>>(p: P) -> Result<Self> {
        ...
        let token2idx: HashMap<String, u32> =
            serde_json::from_reader(file).map_err(lantern::Error::wrap)?;
    ...
    pub fn decode(&self, tokens: &[u32]) -> Result<String> {
        ...
        String::from_utf8(bytes).map_err(lantern::Error::wrap)
    }
    ...
    pub fn encode_bytes(&self, bytes: &[u8]) -> Result<Vec<u32>> {
        ...
        while i < bytes.len() {
            ...
            let token = match self.token2idx.get(&s) {
                None => lantern::bail!("unexpected token '{}' {s:?}", String::from_utf8_lossy(&s)),
                Some(token) => *token,
            };
            tokens.push(token)
        }
