...
use lantern::{Result, D};
use lantern_nn::{batch_norm, Conv2d, Func, VarBuilder};
...
fn conv2d(
    ...
    let conv2d_cfg = lantern_nn::Conv2dConfig {
        stride,
        padding,
        ..Default::default()
    };
    lantern_nn::conv2d_no_bias(c_in, c_out, ksize, conv2d_cfg, vb)
}
...
fn resnet(
    ...
    let fc = match nclasses {
        None => None,
        Some(nclasses) => {
            let linear = lantern_nn::linear(512, nclasses, vb.pp("fc"))?;
            Some(linear)
        }
    };
    Ok(Func::new(move |xs| {
...
fn bottleneck_resnet(
    ...
    let fc = match nclasses {
        None => None,
        Some(nclasses) => {
            let linear = lantern_nn::linear(4 * 512, nclasses, vb.pp("fc"))?;
            Some(linear)
        }
    };
    Ok(Func::new(move |xs| {
