...
use lantern_nn::{layer_norm, LayerNorm, Linear, Module, VarBuilder};
...
fn linear(vb: VarBuilder, in_dim: usize, out_dim: usize, bias: bool) -> Result<Linear> {
    if bias {
        lantern_nn::linear(in_dim, out_dim, vb)
    } else {
        lantern_nn::linear_no_bias(in_dim, out_dim, vb)
    }
}
...
impl Module for Attention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let attn = lantern_nn::ops::softmax(&attn, D::Minus1)?;
...
impl PatchEmbed {
    fn new(vb: VarBuilder, patch_size: usize, in_chans: usize, embed_dim: usize) -> Result<Self> {
        let config = lantern_nn::Conv2dConfig {
            stride: patch_size,
            ..Default::default()
        };
        let proj = lantern_nn::conv2d(in_chans, embed_dim, patch_size, config, vb.pp("proj"))?;
        Ok(Self {
            proj,
...
impl Module for PatchEmbed {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if (h % patch_h) != 0 {
            lantern::bail!("image height {h} is not a multiple of patch height {patch_h}")
        }
        if (w % patch_w) != 0 {
            lantern::bail!("image width {w} is not a multiple of patch width {patch_w}")
        }
