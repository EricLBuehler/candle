use lantern::{DType, Device, Error as E, IndexOp, Module, Result, Tensor, D};
use lantern_nn::{embedding, linear_b, rms_norm_non_quant, Embedding, Linear, RmsNorm, VarBuilder};
...
pub mod speaker_encoder {
    ...
    impl Model {
        pub fn new(cfg: Config, vb: VarBuilder) -> Result<Self> {
            ...
            for layer_idx in 0..cfg.model_num_layers {
                let c = lantern_nn::LSTMConfig {
                    layer_idx,
                    ..Default::default()
                };
                let lstm = lantern_nn::lstm(
                    cfg.mel_n_channels,
        ...
        pub fn embed_utterance(
            ...
            let max_wave_length = match wav_slices.last() {
                Some(v) => v.1,
                None => lantern::bail!("empty wav slices"),
            };
    ...
    impl Module for Model {
        fn forward(&self, xs: &Tensor) -> Result<Tensor> {
            ...
            // This is different from the Python transformers version as lantern LSTM is batch first.
            let xs = xs.t()?;
            let mut xs = xs.clone();
    ...
    impl BPE {
        pub fn from_json(json: &serde_json::Value, end_of_text: usize) -> Result<Self> {
            let json = match json.as_object() {
                None => lantern::bail!("json value is not an object"),
                Some(json) => json,
            };
            let re = match json.get("pat_str") {
                None => lantern::bail!("json object has no pat_str field"),
                Some(pat_str) => match pat_str.as_str() {
                    None => lantern::bail!("pat_str field is not a string"),
                    Some(pat_str) => fancy_regex::Regex::new(pat_str).map_err(E::wrap)?,
                },
            };
            let offset = match json.get("offset") {
                None => lantern::bail!("json object has no offset field"),
                Some(offset) => match offset.as_u64() {
                    None => lantern::bail!("offset field is not a positive int"),
            ...
            let mergeable_ranks = match json.get("mergeable_ranks") {
                None => lantern::bail!("json object has no mergeable_ranks field"),
                Some(mr) => match mr.as_object() {
                    None => lantern::bail!("mergeable_ranks is not an object"),
                    Some(mr) => mr,
                },
            };
            for (key, value) in mergeable_ranks.iter() {
                let value = match value.as_u64() {
                    None => lantern::bail!("mergeable_ranks '{key}' is not a u64"),
...
pub mod gpt {
    use lantern_nn::layer_norm::RmsNormNonQuantized;
    ...
    enum Norm {
        RMSNorm(lantern_nn::RmsNorm<RmsNormNonQuantized>),
        LayerNorm(lantern_nn::LayerNorm),
    }
    ...
    impl Norm {
        fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
            match cfg.norm_type {
                NormType::RMSNorm => {
                    let rms_norm = lantern_nn::rms_norm_non_quant(cfg.n_embd, cfg.rmsnorm_eps, vb)?;
                    Ok(Self::RMSNorm(rms_norm))
                }
                NormType::LayerNorm => {
                    let ln_cfg = lantern_nn::LayerNormConfig {
                        affine: cfg.bias,
                        ..Default::default()
                    };
                    let layer_norm = lantern_nn::layer_norm(cfg.n_embd, ln_cfg, vb)?;
    ...
    impl SelfAttention {
        fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
            ...
            if cfg.attn_kernel_type != AttnKernelType::TorchAttn {
                lantern::bail!("only TorchAttn is supported")
            }
            if cfg.kv_cache_enabled {
                lantern::bail!("kv_cache_enabled=true is not supported")
            }
    ...
    impl Module for SelfAttention {
        fn forward(&self, xs: &Tensor) -> Result<Tensor> {
            ...
            // TODO: causal mask
            let att = lantern_nn::ops::softmax_last_dim(&att)?;
            ...
            let slf = match cfg.nonlinearity_type {
                ...
                NonLinearityType::Swiglu => {
                    ...
                    let swiglu_multiple_of = match cfg.swiglu_multiple_of {
                        None => lantern::bail!("swiglu-multiple-of has to be set"),
                        Some(smo) => smo,
                    };
    ...
    #[allow(clippy::upper_case_acronyms)]
    pub struct Model {
        wtes: Vec<lantern_nn::Embedding>,
        wpe: lantern_nn::Embedding,
        h: Vec<Block>,
        ln_f: Norm,
        lm_heads: Vec<Linear>,
        cfg: Config,
    ...
    impl Model {
        pub fn new(cfg: Config, vb: VarBuilder) -> Result<Self> {
            ...
            for (idx, vocab_size) in cfg.vocab_sizes.iter().enumerate() {
                let wte = lantern_nn::embedding(*vocab_size, cfg.n_embd, vb_w.pp(idx))?;
                wtes.push(wte)
            }
            let wpe = lantern_nn::embedding(cfg.block_size, cfg.n_embd, vb_t.pp("wpe"))?;
...
pub mod transformer {
    ...
    impl Module for FeedForward {
        fn forward(&self, xs: &Tensor) -> Result<Tensor> {
            let _enter = self.span.enter();
            let swiglu = (lantern_nn::ops::silu(&xs.apply(&self.w1)?)? * xs.apply(&self.w3))?;
    ...
    impl Attention {
        ...
        fn forward(&mut self, xs: &Tensor, _pos: usize, mask: &Tensor) -> Result<Tensor> {
            ...
            let attn_weights = lantern_nn::ops::softmax_last_dim(&attn_weights)?;
