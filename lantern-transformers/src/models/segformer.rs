...
use lantern::{Module, ModuleT, Result, Tensor, D};
use lantern_nn::{conv2d_no_bias, layer_norm, Activation, Conv2dConfig, VarBuilder};
...
pub struct Config {
    ...
    pub hidden_act: lantern_nn::Activation,
...
struct SegformerOverlapPatchEmbeddings {
    projection: Conv2d,
    layer_norm: lantern_nn::LayerNorm,
}
...
impl SegformerOverlapPatchEmbeddings {
    fn new(
        ...
        let layer_norm =
            lantern_nn::layer_norm(hidden_size, config.layer_norm_eps, vb.pp("layer_norm"))?;
        Ok(Self {
            projection,
            layer_norm,
        })
...
impl Module for SegformerEfficientSelfAttention {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let attention_scores = lantern_nn::ops::softmax_last_dim(&attention_scores)?;
...
struct SegformerLayer {
    layer_norm_1: lantern_nn::LayerNorm,
    attention: SegformerAttention,
    layer_norm_2: lantern_nn::LayerNorm,
    mlp: SegformerMixFFN,
}
...
struct SegformerEncoder {
    ...
    layer_norms: Vec<lantern_nn::LayerNorm>,
}
...
struct SegformerDecodeHead {
    ...
    linear_fuse: lantern_nn::Conv2d,
    batch_norm: lantern_nn::BatchNorm,
    classifier: lantern_nn::Conv2d,
}
...
impl SegformerDecodeHead {
    fn new(config: &Config, num_labels: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let batch_norm = lantern_nn::batch_norm(
            config.decoder_hidden_size,
            config.layer_norm_eps,
            vb.pp("batch_norm"),
        )?;
