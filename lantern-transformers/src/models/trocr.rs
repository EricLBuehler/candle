...
use lantern::{DType, Result, Tensor};
use lantern_nn::{
...
pub struct TrOCRConfig {
    ...
    pub activation_function: lantern_nn::Activation,
...
impl Default for TrOCRConfig {
    fn default() -> Self {
        Self {
            ...
            activation_function: lantern_nn::Activation::Gelu,
...
impl TrOCRAttention {
    ...
    fn forward(
        ...
        let attn_probs = lantern_nn::ops::softmax_last_dim(&attn_weights)?;
...
struct TrOCRDecoderLayer {
    ...
    activation_fn: lantern_nn::Activation,
...
impl TrOCRForCausalLM {
    pub fn new(decoder_cfg: &TrOCRConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let output_projection = if decoder_cfg.tie_word_embeddings {
            lantern_nn::Linear::new(decoder.embed_tokens.embeddings().clone(), None)
        } else {
            lantern_nn::linear_no_bias(
                decoder_cfg.d_model,
