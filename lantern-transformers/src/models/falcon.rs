...
use lantern_nn::{embedding, linear_b as linear, Embedding, LayerNorm, Linear, Module, VarBuilder};
...
impl Config {
    pub fn validate(&self) -> Result<()> {
        if self.alibi {
            lantern::bail!("alibi is not supported");
        }
        if self.new_decoder_architecture {
            lantern::bail!("new_decoder_architecture is not supported");
        }
        if self.n_head_kv.is_some() {
            lantern::bail!("n_head_kv is not supported");
        }
        Ok(())
    }
...
impl FalconAttention {
    ...
    fn forward(&mut self, x: &Tensor, mask: Option<&Tensor>, past_kv_len: usize) -> Result<Tensor> {
        ...
        let attention_scores =
            lantern_nn::ops::softmax(&attention_scores.to_dtype(DType::F32)?, D::Minus1)?
                .to_dtype(x.dtype())?;
