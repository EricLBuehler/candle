...
let weights = lantern::safetensors::load(weights_filename, &Device::Cpu).unwrap();
...
let weights = lantern::safetensors::load_buffer(&mmap[..], &Device::Cpu).unwrap();
...
// Convert safetensors Dtype to lantern DType
...
let mnist = lantern_datasets::vision::Dataset {
    train_images,
    train_labels,
    test_images,
    test_labels,
