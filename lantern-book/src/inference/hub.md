...
let weights = lantern_core::safetensors::load(weights, &Device::Cpu);
```
...
# 
# let weights = repo.get("model.safetensors").unwrap();
use lantern_core::{Device, Tensor, DType};
use lantern_nn::{Linear, Module};

let weights = lantern_core::safetensors::load(weights, &Device::Cpu).unwrap();
...
For a full reference, you can check out the full [bert](https://github.com/LaurentMazare/lantern/tree/main/lantern-examples/examples/bert) example.
