...
use ::lantern::{quantized::QTensor, DType, Device, Module, Tensor, WithDType};
...
fn actual_index(t: &Tensor, dim: usize, index: i64) -> ::lantern::Result<usize> {
    let dim = t.dim(dim)?;
    if 0 <= index {
        let index = index as usize;
        if dim <= index {
            ::lantern::bail!("index {index} is too large for tensor dimension {dim}")
        }
        Ok(index)
    } else {
        if (dim as i64) < -index {
            ::lantern::bail!("index {index} is too low for tensor dimension {dim}")
        }
        Ok((dim as i64 + index) as usize)
    }
}

fn actual_dim(t: &Tensor, dim: i64) -> ::lantern::Result<usize> {
    let rank = t.rank();
    if 0 <= dim {
        let dim = dim as usize;
        if rank <= dim {
            ::lantern::bail!("dimension index {dim} is too large for tensor rank {rank}")
        }
        Ok(dim)
    } else {
        if (rank as i64) < -dim {
            ::lantern::bail!("dimension index {dim} is too low for tensor rank {rank}")
        }
        Ok((rank as i64 + dim) as usize)
    }
}

// TODO: Something similar to this should probably be a part of lantern core.
trait MapDType {
    type Output;
...
impl PyTensor {
    ...
    /// Converts lantern's tensor to pytorch's tensor
    /// &RETURNS&: torch.Tensor
    fn to_torch(&self, py: Python<'_>) -> PyResult<PyObject> {
        let lantern_values = self.values(py)?;
        let torch_tensor: PyObject = py
            .import_bound("torch")?
            .getattr("tensor")?
            .call1((lantern_values,))?
            .extract()?;
        Ok(torch_tensor)
    }
    ...
    fn __richcmp__(&self, rhs: &Bound<PyAny>, op: CompareOp) -> PyResult<Self> {
        ...
        if let Ok(rhs) = rhs.extract::<PyTensor>() {
            if self.0.shape() == rhs.0.shape() {
                ...
                // We broadcast manually here because `lantern.cmp` does not support automatic broadcasting
...
impl PyQTensor {
    ...
    fn matmul_t(&self, lhs: &PyTensor) -> PyResult<PyTensor> {
        let qmatmul = ::lantern::quantized::QMatMul::from_arc(self.0.clone()).map_err(wrap_err)?;
...
fn load_safetensors(path: &str, py: Python<'_>) -> PyResult<PyObject> {
    let res = ::lantern::safetensors::load(path, &Device::Cpu).map_err(wrap_err)?;
    let res = res
        .into_iter()
...
fn save_safetensors(
    ...
    ::lantern::safetensors::save(&tensors, path).map_err(wrap_err)
}
...
fn load_ggml(
    ...
    let device = device.unwrap_or(PyDevice::Cpu).as_device()?;
    let ggml =
        ::lantern::quantized::ggml_file::Content::read(&mut file, &device).map_err(wrap_err)?;
    let tensors = ggml
        .tensors
        .into_iter()
        .map(|(key, qtensor)| Ok((key, PyQTensor(Arc::new(qtensor)).into_py(py))))
        .collect::<::lantern::Result<Vec<_>>>()
        .map_err(wrap_err)?;
...
fn load_gguf(
    ...
    let device = device.unwrap_or(PyDevice::Cpu).as_device()?;
    use ::lantern::quantized::gguf_file;
...
fn cuda_is_available() -> bool {
    ::lantern::utils::cuda_is_available()
}

#[pyfunction]
/// Returns true if lantern was compiled with 'accelerate' support.
/// &RETURNS&: bool
fn has_accelerate() -> bool {
    ::lantern::utils::has_accelerate()
}

#[pyfunction]
/// Returns true if lantern was compiled with MKL support.
/// &RETURNS&: bool
fn has_mkl() -> bool {
    ::lantern::utils::has_mkl()
}

#[pyfunction]
/// Returns the number of threads used by the lantern.
/// &RETURNS&: int
fn get_num_threads() -> usize {
    ::lantern::utils::get_num_threads()
}

fn lantern_utils(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
fn softmax(tensor: PyTensor, dim: i64) -> PyResult<PyTensor> {
    ...
    let sm = lantern_nn::ops::softmax(&tensor.0, dim).map_err(wrap_err)?;
    Ok(PyTensor(sm))
}
...
fn silu(tensor: PyTensor) -> PyResult<PyTensor> {
    let s = lantern_nn::ops::silu(&tensor.0).map_err(wrap_err)?;
    Ok(PyTensor(s))
}
...
fn lantern_functional_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
#[cfg(feature = "onnx")]
fn lantern_onnx_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
#[pymodule]
fn lantern(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    let utils = PyModule::new_bound(py, "utils")?;
    lantern_utils(py, &utils)?;
    m.add_submodule(&utils)?;
    let nn = PyModule::new_bound(py, "functional")?;
    lantern_functional_m(py, &nn)?;
    m.add_submodule(&nn)?;
    #[cfg(feature = "onnx")]
    {
        let onnx = PyModule::new_bound(py, "onnx")?;
        lantern_onnx_m(py, &onnx)?;
