from lantern.utils import load_safetensors, save_gguf, load_gguf
from lantern.models.bert import BertModel, Config
import json
...
if __name__ == "__main__":
    ...
    lantern_pooled = average_pool(torch.tensor(encoder_out.values()), hf_tokenized["attention_mask"])

    loss = torch.nn.L1Loss()
    error = loss(hf_pooled, lantern_pooled).mean().item()
    print(f"Mean error between torch-reference and lantern: {error}")
    ...
    lantern_pooled_2 = average_pool(torch.tensor(encoder_out_2.values()), hf_tokenized["attention_mask"])
    error = loss(hf_pooled, lantern_pooled_2).mean().item()
    print(f"Mean error between torch-reference and quantized-lantern: {error}")
