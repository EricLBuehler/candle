...
_UNSIGNED_DTYPES = set([str(lantern.u8), str(lantern.u32)])
...
def assert_almost_equal(
    ...
    if str(actual.dtype) in _UNSIGNED_DTYPES or str(expected.dtype) in _UNSIGNED_DTYPES:
        actual = actual.to(lantern.i64)
        expected = expected.to(lantern.i64)

    diff = (actual - expected).abs()

    threshold = (expected.abs().to_dtype(lantern.f32) * rtol + atol).to(expected)
