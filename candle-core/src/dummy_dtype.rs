    ...
    ($ty:ty, $dtype:ident) => {
        impl WithDType for $ty {
            ...
            fn from_f64(_v: f64) -> Self {
                panic!("{} is a dummy type and cannot be constructed", stringify!($ty))
            }

            fn to_f64(self) -> f64 {
                panic!("{} is a dummy type and cannot be converted", stringify!($ty))
            }
            
            fn to_scalar(self) -> crate::scalar::Scalar {
                panic!("{} is a dummy type and cannot be converted to scalar", stringify!($ty))
            }
    ...
    ($ty:ty) => {
        impl std::ops::AddAssign for $ty {
            fn add_assign(&mut self, _other: Self) {
                panic!("{} is a dummy type and does not support operations", stringify!($ty))
            }
        }
        ...
        impl num_traits::Num for $ty {
            ...
            fn from_str_radix(_str: &str, _radix: u32) -> std::result::Result<Self, Self::FromStrRadixErr> {
                panic!("{} is a dummy type and does not support parsing", stringify!($ty))
            }
        }
