...
use lantern::{bail, DType, Device, Result, Tensor};
...
impl Attr for str {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<&Self> {
        std::str::from_utf8(&attr.s).map_err(lantern::Error::wrap)
    }
}
...
impl Attr for GraphProto {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<&Self> {
        attr.g
            .as_ref()
            .ok_or_else(|| lantern::Error::Msg("attribute does not contain graph".to_string()))
    }
}
        ...
        match node.op_type.as_str() {
            ...
            "LogSoftmax" => {
                ...
                let output = match get_attr_opt::<i64>(node, "axis")? {
                    None => lantern_nn::ops::softmax_last_dim(input)?,
                    Some(&axis) => {
                        let axis = input.normalize_axis(axis)?;
                        lantern_nn::ops::log_softmax(input, axis)?
                    }
                };
                values.insert(node.output[0].clone(), output);
            }
            "Softmax" => {
                ...
                let output = match get_attr_opt::<i64>(node, "axis")? {
                    ...
                    Some(&axis) => {
                        let axis = input.normalize_axis(axis)?;
                        lantern_nn::ops::softmax(input, axis)?
                    }
                };
                values.insert(node.output[0].clone(), output);
            }
            ...
            "Gather" => {
                ...
                // tensor directly, but lantern does not support tensor indexing at the moment, so
            ...
            "Sigmoid" => {
                let input = get(&node.input[0])?;
                let output = lantern_nn::ops::sigmoid(input)?;
                values.insert(node.output[0].clone(), output);
            }
            "Gelu" => {
                let input = get(&node.input[0])?;
            ...
            "LeakyRelu" => {
                ...
                let output = lantern_nn::ops::leaky_relu(input, alpha.into())?;
                values.insert(node.output[0].clone(), output);
            }
