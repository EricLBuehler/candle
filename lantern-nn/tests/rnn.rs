...
use lantern::{test_utils::to_vec2_round, DType, Device, Result, Tensor};
use lantern_nn::RNN;
...
fn lstm() -> Result<()> {
    ...
    .into_iter()
    .collect();
    let vb = lantern_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let lstm = lantern_nn::lstm(2, 3, Default::default(), vb)?;
...
fn gru() -> Result<()> {
    ...
    .collect();
    let vb = lantern_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let gru = lantern_nn::gru(2, 3, Default::default(), vb)?;
