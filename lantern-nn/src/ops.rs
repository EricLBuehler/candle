use lantern::{CpuStorage, DType, Layout, Module, Result, Shape, Tensor, D};
use rayon::prelude::*;
...
///
/// ```rust
/// use lantern::{Tensor, Device, test_utils::to_vec2_round};
/// let a = Tensor::new(&[[0f32, 1., 0., 1.], [-2., 2., 3., -3.]], &Device::Cpu)?;
/// let a = lantern_nn::ops::softmax(&a, 1)?;
/// assert_eq!(
///     to_vec2_round(&a, 4)?,
///     &[
...
/// # Ok::<(), lantern::Error>(())
/// ```
pub fn softmax<D: lantern::shape::Dim>(xs: &Tensor, dim: D) -> Result<Tensor> {
...
pub fn log_softmax<D: lantern::shape::Dim>(xs: &Tensor, d: D) -> Result<Tensor> {
...
impl lantern::CustomOp1 for Sigmoid {
    fn name(&self) -> &'static str {
        "sigmoid"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        use lantern::backend::BackendStorage;

        fn fwd<T: num_traits::Float>(v: T) -> T {
            (v.neg().exp() + T::one()).recip()
        }

        // FIXME: using `lantern::map_dtype` causes compilation errors.
        let storage = match storage {
            CpuStorage::BF16(slice) => {
                CpuStorage::BF16(lantern::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F16(slice) => {
                CpuStorage::F16(lantern::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F32(slice) => {
                CpuStorage::F32(lantern::cpu_backend::unary_map(slice, layout, fwd))
            }
            CpuStorage::F64(slice) => {
                CpuStorage::F64(lantern::cpu_backend::unary_map(slice, layout, fwd))
            }
            _ => Err(lantern::Error::UnsupportedDTypeForOp(
                storage.dtype(),
                self.name(),
            ))?,
        };
    ...
    fn cuda_fwd(
        &self,
        storage: &lantern::CudaStorage,
        layout: &Layout,
    ) -> Result<(lantern::CudaStorage, Shape)> {
        use lantern::backend::BackendStorage;
        use lantern::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchAsync, LaunchConfig, ValidAsZeroBits,
        };
        use lantern::cuda_backend::SlicePtrOrNull;
        use lantern::cuda_backend::{kernel_name, kernels, Map1, WrapErr};
        use lantern::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        &self,
        storage: &lantern::MetalStorage,
        layout: &Layout,
    ) -> Result<(lantern::MetalStorage, Shape)> {
        use lantern::backend::BackendStorage;
        use lantern::MetalError;
        ...
        let src = lantern_metal_kernels::BufferOffset {
        ...
        match (el_count % 2, dtype, layout.is_contiguous()) {
            (0, DType::BF16 | DType::F16, true) => {
                use lantern_metal_kernels::unary::contiguous_tiled;
                ...
                lantern_metal_kernels::call_unary_contiguous_tiled(
                    device.metal_device(),
                    &command_buffer,
                    device.kernels(),
                    kernel_name,
            ...
            (_, _, true) => {
                use lantern_metal_kernels::unary::contiguous;
                let kernel_name = match dtype {
                    DType::F16 => contiguous::sigmoid::HALF,
                    DType::F32 => contiguous::sigmoid::FLOAT,
                    DType::BF16 => contiguous::sigmoid::BFLOAT,
                    dtype => {
                        lantern::bail!("Metal contiguous unary sigmoid {dtype:?} not implemented")
                    }
                };
                lantern_metal_kernels::call_unary_contiguous(
                    device.metal_device(),
                    &command_buffer,
                    device.kernels(),
                    kernel_name,
            ...
            (_, _, false) => {
                use lantern_metal_kernels::unary::strided;
                let kernel_name = match dtype {
                    DType::F16 => strided::sigmoid::HALF,
                    DType::F32 => strided::sigmoid::FLOAT,
                    DType::BF16 => strided::sigmoid::BFLOAT,
                    dtype => {
                        lantern::bail!("Metal strided unary sigmoid {dtype:?} not implemented")
                    }
                };
                let dst = lantern_metal_kernels::BufferOffset::zero_offset(&buffer);
                lantern_metal_kernels::call_unary_strided(
                    device.metal_device(),
                    &command_buffer,
                    device.kernels(),
                    kernel_name,
        ...
        let new_storage = lantern::MetalStorage::new(buffer, device.clone(), el_count, dtype);
...
pub fn dropout(xs: &Tensor, drop_p: f32) -> Result<Tensor> {
    ...
    if !(0. ..1.).contains(&drop_p) {
        lantern::bail!("dropout probability has to be in [0, 1), got {drop_p}")
    }
...
impl lantern::ModuleT for Dropout {
...
impl lantern::CustomOp1 for SoftmaxLastDim {
    fn name(&self) -> &'static str {
        "softmax-last-dim"
    }

    fn cpu_fwd(&self, storage: &CpuStorage, layout: &Layout) -> Result<(CpuStorage, Shape)> {
        fn softmax<T: lantern::WithDType + num_traits::Float>(
            src: &[T],
            layout: &Layout,
        ) -> Result<(CpuStorage, Shape)> {
            let src = match layout.contiguous_offsets() {
                None => lantern::bail!("input has to be contiguous"),
            ...
            let storage = lantern::WithDType::to_cpu_storage_owned(dst);
        ...
        match storage {
            ...
            _ => lantern::bail!("unsupported dtype for softmax {:?}", storage),
        }
    }
    ...
    fn cuda_fwd(
        ...
        let dev = storage.device();
        let slice = S.map(&storage.slice, dev, layout)?;
        let dst = lantern::cuda_backend::CudaStorage {
            slice,
            device: dev.clone(),
        };
        Ok((dst, layout.shape().clone()))
    ...
    fn metal_fwd(
        ...
        let name = match storage.dtype() {
            ...
            dtype => lantern::bail!("softmax-last-dim is not implemented for {dtype:?}"),
        };

        let n = layout.stride().len();
        if !(layout.is_contiguous() && layout.stride()[n - 1] == 1) {
            lantern::bail!("Non contiguous softmax-last-dim is not implemented");
        }

        let last_dim = layout.dims()[layout.shape().rank() - 1];
        let elem_count = layout.shape().elem_count();
        let output = device.new_buffer(elem_count, storage.dtype(), "softmax")?;
        lantern_metal_kernels::call_last_softmax(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
        ...
        .map_err(lantern::Error::wrap)?;
        let newstorage =
            lantern::MetalStorage::new(output, device.clone(), elem_count, storage.dtype());
...
impl lantern::CustomOp2 for RmsNorm {
    fn name(&self) -> &'static str {
        "rms-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let alpha = match alpha_layout.contiguous_offsets() {
                None => lantern::bail!("alpha has to be contiguous"),
        ...
        match (s1, s2) {
            ...
            _ => lantern::bail!("unsupported dtype for rmsnorm {:?}", s1.dtype()),
        }
    }
    ...
    fn cuda_fwd(
        &self,
        s1: &lantern::CudaStorage,
        l1: &Layout,
        s2: &lantern::CudaStorage,
        l2: &Layout,
    ) -> Result<(lantern::CudaStorage, Shape)> {
        use lantern::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchAsync, LaunchConfig,
        };
        use lantern::cuda_backend::{kernel_name, kernels, Map2, WrapErr};
        use lantern::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        &self,
        s1: &lantern::MetalStorage,
        l1: &Layout,
        s2: &lantern::MetalStorage,
        l2: &Layout,
    ) -> Result<(lantern::MetalStorage, Shape)> {
        use lantern::backend::BackendStorage;
        ...
        let name = match (s1.dtype(), s2.dtype()) {
            ...
            (dt1, dt2) => lantern::bail!("rmsnorm is not implemented for {dt1:?} {dt2:?}"),
        };

        if !(l1.is_contiguous() && l2.is_contiguous()) {
            lantern::bail!("Non contiguous rmsnorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "rmsnorm")?;
        lantern_metal_kernels::call_rms_norm(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
        ...
        .map_err(lantern::Error::wrap)?;
        let newstorage = lantern::MetalStorage::new(output, device.clone(), elem_count, s1.dtype());
...
impl lantern::CustomOp3 for LayerNorm {
    fn name(&self) -> &'static str {
        "layer-norm"
    }
    ...
    fn cpu_fwd(
        ...
        >(
            ...
            let beta = match beta_layout.contiguous_offsets() {
                None => lantern::bail!("beta has to be contiguous"),
    ...
    fn cuda_fwd(
        ...
        s2: &lantern::CudaStorage,
        l2: &Layout,
        s3: &lantern::CudaStorage,
        l3: &Layout,
    ) -> Result<(lantern::CudaStorage, Shape)> {
        use lantern::cuda_backend::cudarc::driver::{
            CudaSlice, DeviceRepr, LaunchAsync, LaunchConfig,
        };
        use lantern::cuda_backend::{kernel_name, kernels, Map3, WrapErr};
        use lantern::{CudaDevice, WithDType};
    ...
    fn metal_fwd(
        ...
        let name = match (s1.dtype(), s2.dtype(), s3.dtype()) {
            ...
            (dt1, dt2, dt3) => {
                lantern::bail!("layernorm is not implemented for {dt1:?} {dt2:?} {dt3:?}")
            }
        };

        if !(l1.is_contiguous() && l2.is_contiguous() && l3.is_contiguous()) {
            lantern::bail!("Non contiguous layernorm is not implemented");
        }

        let last_dim = l1.dims()[l1.shape().rank() - 1];
        let elem_count = l1.shape().elem_count();
        let output = device.new_buffer(elem_count, s1.dtype(), "layernorm")?;
        lantern_metal_kernels::call_layer_norm(
            device.metal_device(),
            &command_buffer,
            kernels,
            name,
...
pub fn replication_pad2d(xs: &Tensor, pad: usize) -> Result<Tensor> {
    match pad {
        ...
        n => lantern::bail!("replication-pad with a size of {n} is not supported"),
    }
}
...
pub fn kvconcat(ltensor: &Tensor, rtensor: &Tensor, concat_dim: usize) -> Result<Tensor> {
    ...
    use lantern::cuda_backend::KVConcat;
