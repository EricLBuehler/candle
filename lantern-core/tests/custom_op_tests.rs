use lantern_core::backend::BackendStorage;
use lantern_core::cpu_backend;
use lantern_core::test_utils::to_vec1_round;
use lantern_core::{CpuStorage, CustomOp1, DType, Device, Error, Layout, Result, Shape, Tensor};
...
impl CustomOp1 for Elu {
    ...
    fn cpu_fwd(&self, s: &CpuStorage, l: &Layout) -> Result<(CpuStorage, Shape)> {
        let storage = lantern_core::map_dtype!(
            "elu",
            s,
...
fn custom_op1_with_backward() -> Result<()> {
    let cpu = &Device::Cpu;
    let t = lantern_core::Var::new(&[-2f32, 0f32, 2f32], cpu)?;
...
impl lantern_core::InplaceOp1 for Elu {
    fn name(&self) -> &'static str {
        "elu"
    }
    ...
    fn cpu_fwd(&self, s: &mut CpuStorage, _l: &Layout) -> Result<()> {
        ...
        match s {
            ...
            _ => lantern_core::bail!("unsupported dtype for inplace elu"),
        }
        Ok(())
    }
}
