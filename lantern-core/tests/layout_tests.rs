use lantern::{test_device, Device, IndexOp, Result, Tensor};
use lantern_core as lantern;
...
fn strided_blocks() -> Result<()> {
    use lantern::Device::Cpu;
    let tensor = Tensor::arange(0u32, 24u32, &Cpu)?.reshape((2, 3, 4))?;
    match tensor.strided_blocks() {
        lantern::StridedBlocks::SingleBlock { start_offset, len } => {
            assert_eq!(start_offset, 0);
            assert_eq!(len, 24);
        }
        lantern::StridedBlocks::MultipleBlocks { .. } => {
    ...
    let tensor = tensor.i((.., 1))?;
    match tensor.strided_blocks() {
        lantern::StridedBlocks::SingleBlock { .. } => {
            panic!("unexpected block structure")
        }
        lantern::StridedBlocks::MultipleBlocks {
            block_len,
