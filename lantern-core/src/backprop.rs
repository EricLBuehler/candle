...
thread_local! {
    static LANTERN_GRAD_DO_NOT_DETACH: bool = {
        match std::env::var("LANTERN_GRAD_DO_NOT_DETACH") {
            Ok(s) => {
                !s.is_empty() && s != "0"
            },
            Err(_) => false,
...
impl Tensor {
    ...
    pub fn backward(&self) -> Result<GradStore> {
        ...
        for node in sorted_nodes.iter() {
            ...
            let grad = grads
                .remove(node)
                .expect("lantern internal error - grad not populated");
            // https://github.com/huggingface/lantern/issues/1241
            ...
            let do_not_detach = LANTERN_GRAD_DO_NOT_DETACH.with(|b| *b);
