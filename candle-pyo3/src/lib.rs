...
trait MapDType {
    ...
    fn map(&self, t: &Tensor) -> PyResult<Self::Output> {
        match t.dtype() {
            ...
            DType::I16 => Err(PyErr::new::<PyTypeError, _>("i16 dtype is not supported in Python interface")),
            DType::I32 => Err(PyErr::new::<PyTypeError, _>("i32 dtype is not supported in Python interface")),
            DType::F8E4M3 => Err(PyErr::new::<PyTypeError, _>("f8e4m3 dtype is not supported in Python interface")),
            DType::F6E2M3 | DType::F6E3M2 | DType::F4 | DType::F8E8M0 => {
                Err(PyErr::new::<PyTypeError, _>(format!("Dummy dtype {:?} is not supported", t.dtype())))
            }
        }
    }
}
