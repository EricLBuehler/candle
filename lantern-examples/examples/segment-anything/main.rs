...
use lantern_transformers::models::segment_anything::sam;
...
pub fn main() -> anyhow::Result<()> {
    ...
    let (image, initial_h, initial_w) =
        lantern_examples::load_image(&args.image, Some(sam::IMAGE_SIZE))?;
    ...
    let model = match args.model {
        ...
        None => {
            ...
            let api = api.model("lmz/lantern-sam".to_string());
    ...
    if args.generate_masks {
        ...
        for (idx, bbox) in bboxes.iter().enumerate() {
            ...
            lantern_examples::save_image_resize(
                &mask,
