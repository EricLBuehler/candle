...
use lantern_transformers::generation::{LogitsProcessor, Sampling};
use lantern_transformers::models::llama::Cache;
...
use lantern_transformers::models::llava::config::{
    HFGenerationConfig, HFLLaVAConfig, HFPreProcessorConfig,
};
use lantern_transformers::models::llava::{config::LLaVAConfig, LLaVA};
use std::io::Write;
...
struct Args {
    ...
    #[arg(long)]
    prompt: String,
    /// The seed to use when generating random samples. Copy from lantern llama. Not exist in python llava.
    #[arg(long, default_value_t = 299792458)]
    seed: u64,
}

//from https://github.com/huggingface/lantern/blob/main/lantern-examples/examples/clip/main.rs
...
fn main() -> Result<()> {
    ...
    let weight_filenames =
        lantern_examples::hub_load_safetensors(&api, "model.safetensors.index.json")?;
    ...
    //inference loop, based on https://github.com/huggingface/lantern/blob/main/lantern-examples/examples/llama/main.rs
    let mut tokenizer = lantern_examples::token_output_stream::TokenOutputStream::new(tokenizer);
    let mut index_pos = 0;
