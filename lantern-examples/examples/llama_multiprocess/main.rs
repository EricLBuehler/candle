...
fn main() -> Result<()> {
    ...
    let filenames = lantern_examples::hub_load_safetensors(&api, "model.safetensors.index.json")?;
    ...
    let vb = unsafe {
        lantern_nn::var_builder::ShardedSafeTensors::var_builder(&filenames, dtype, &device)?
    };
