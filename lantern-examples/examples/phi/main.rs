...
use lantern_transformers::models::mixformer::{
    Config, MixFormerSequentialForCausalLM as MixFormer,
};
use lantern_transformers::models::phi::{Config as PhiConfig, Model as Phi};
use lantern_transformers::models::phi3::{Config as Phi3Config, Model as Phi3};
use lantern_transformers::models::quantized_mixformer::MixFormerSequentialForCausalLM as QMixFormer;

use hf_hub::{api::sync::Api, Repo, RepoType};
use lantern::{DType, Device, IndexOp, Tensor};
...
fn main() -> Result<()> {
    ...
    let model_id = match args.model_id {
        ...
        None => {
            if args.quantized {
                "lmz/lantern-quantized-phi".to_string()
            } else {
                match args.model {
            ...
            if args.quantized {
                ...
                match args.model {
                    ...
                    WhichModel::V2 | WhichModel::V2Old | WhichModel::V3 | WhichModel::V3Medium => {
                        lantern_examples::hub_load_safetensors(
                            &repo,
