...
/// (3, 378, 378).
pub fn load_image<P: AsRef<std::path::Path>>(p: P) -> lantern::Result<Tensor> {
    ...
    (data.to_dtype(lantern::DType::F32)? / 255.)?
        .broadcast_sub(&mean)?
        .broadcast_div(&std)
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let model_id = match args.model_id {
        ...
        None => {
            if args.quantized {
                "santiagomed/lantern-moondream".to_string()
            } else {
